...
import common.H2Helper.connString
...
trait H2Helper extends BasicLogUtil with AutoClose {
  ...
  protected def getStmt(implicit conn: Connection): Statement = conn.createStatement()
...
object H2Helper {
  ...
  val connString: String = ConfigManager.dbType match {
    ...
    case FileDBType() =>
      val resolvedPath = (ConfigManager.dbFilePath + "/").replace("//", "/") + "PowlyPollington.h2db"
